...
const ManagerOrderingModal: React.FC<Props> = () => {
  const {
    createSkuMasterList,
    updateSku,
    modalData,
  ...
  const { zSkuAddEditTitle, zBuyerList } = useSkuEnrollmentContext();
  ...
  const onSubmit = async (data: SkuEnrollmentFormValues) => {
    let data2: SkuMasterModel = {
      ...
      ShelfLifeWeeks: data.shelfLifeWeeks.toString(),
...
export default ManagerOrderingModal;
