...
import SkuEnrollmentModal from "./ManagerOrderingModal";
...
const ManagerOrderingTblData: React.FC<TableDataProps> = ({
  data,
  page,
  rowsPerPage,
  onPageChange,
  ...
  const { zSkuMasterList, zSkuSearchText, updateWorksheetfileOrdering } =
    useManagerOrderingContext();
      ...
      if (result.isConfirmed) {
        ...
        for (let i = 0; i < Object.keys(zCheckedItemsCache).length; i++) {
          ...
          let worksheetFileOrderingObj: UpdateWorksheetfileOrderingRequest = {
            SkuNumber: skuNumber,
            ForOrderingStatus:
              ForOrderingStatus && statusType === "approve" ? 3 : 2,
          };
  ...
  const exportToExcel = (data: any[], fileName: string) => {
    ...
    const headerMapping: { [key: string]: string } = {
      ...
      UnitsPerContainer: "Unit per case",
  ...
  const loadMoreRows = () => {
    ...
    const newCheckedState = data
      .slice(0, startIndex + rowsPerPage)
      .reduce((acc, row) => {
        if (row.ForOrderingStatus === 3) {
  ...
  const sortedRows = [...data.slice(0, rowsPerPage * pageScroll)].sort(
    (a, b) => {
      if (!orderBy) return 0;
    ...
    <React.Fragment>
      ...
      <Grid container spacing={1} sx={{ ml: 1.5, mb: 2 }}>
        <Grid item xs={12} sm={3} md={3} xl={3}>
          <Button
            variant="outlined"
            onClick={() => handleApproveStatus("approve")}
          >
            Approve
          </Button>
          <Button
            variant="outlined"
            onClick={() => handleApproveStatus("decline")}
            sx={{ ml: 2 }}
          >
            Decline
          </Button>
...
export default ManagerOrderingTblData;
