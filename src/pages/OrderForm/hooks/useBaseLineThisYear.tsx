...
const useBaseLineThisYear = (initialValue: string) => {
  const { zSetBaseLineThisYear } = useOrderFormContext();
  const [baseLineThisYear, setBaseLineThisYear] = useState(0);
  ...
  const updateBaseLineThisYear = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const filteredValueNum = Number.isNaN(parseInt(filteredValue)) ? 0 : parseInt(filteredValue);

      setBaseLineThisYear(filteredValueNum); // Update the state with the parsed value (filteredValue);
  ...
  const debouncedBTYOnChange = useCallback(
    debounce(updateBaseLineThisYear, 200), // Debouncing for 200ms
    [updateBaseLineThisYear]
  );

  return { baseLineThisYear, debouncedBTYOnChange };
};
